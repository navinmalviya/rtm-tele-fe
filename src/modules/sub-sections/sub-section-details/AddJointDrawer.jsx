...
import { useAddJoint, useCableDetails } from '@/hooks/cable';
...
export const ADD_CABLE_JOINT_DRAWER = 'addCableJointDrawer';
...
export default function AddJointDrawer({ cableId, jointType = 'NORMAL', onClose }) {
	const dispatch = useDispatch();
	const showToast = useToast();
	const { data: cable } = useCableDetails(cableId);
	...
	const handleSave = (values) => {
		...
		if (jointType === 'EC' && !values.ecSocketId) {
		...
		addJoint(
			{
				...
				ecSocketId: jointType === 'EC' ? values.ecSocketId : null,
		...
		<RtmDrawer drawerName={ADD_CABLE_JOINT_DRAWER} onCancel={handleClose}>
			...
			>
				<Box sx={{ p: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
					<Box>
						<Typography variant="h6" sx={{ fontWeight: 800, color: 'text.primary' }}>
							{jointType === 'EC' ? 'Add EC Joint' : 'Add Cable Joint'}
						</Typography>
				...
				<Box sx={{ p: 3, flex: 1, overflowY: 'auto', bgcolor: 'background.default' }}>
					<form id="add-joint-form" onSubmit={handleSubmit(handleSave)}>
						<Stack spacing={2.5}>
							<Typography
								variant="subtitle2"
									...
									render={({ field }) => (
										<TextField
											...
											InputProps={{
												startAdornment: (
													<InputAdornment position="start">
														<CallSplit sx={{ color: 'text.secondary' }} />
													</InputAdornment>
												),
											}}
										/>
